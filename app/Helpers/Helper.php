...
if (!function_exists('renderMenu')) {
    function renderMenu($menuItems)
    {
        ...
        foreach ($menuItems as $item) {
            if (isset($item['label'])) {
                $output .= '<li class="header">' . $item['label'] . '</li>';
                continue;
            }
            ...
            $output .= '<span>' . translate($item['text']) . '</span>';
...
if (!function_exists('checkPermissions')) {
    function checkPermissions($permissions)
    {
        ...
        if (\Auth::user()->hasPermissionTo($permissions[0])) {
            return true;
        }
        return false;
    }
...
if (!function_exists('isRequired')) {
    function isRequired()
    {
        $output = '(<span class="text-danger">required *</span>)';
        return $output;
    }
}

if (!function_exists('getMemoryOS')) {
    function getMemoryOS() {
        echo exec("Total RAM: $(free -h | awk '/^Mem:/ {print $2}') / Used RAM: $(free -h | awk '/^Mem:/ {print $3}')");
    }
}

if (!function_exists('getCPUOS')) {
    function getCPUOS() {
        echo "CPU User: $(top -b -n1 | grep 'Cpu(s)' | awk '{print $2}') / CPU System: $(top -b -n1 | grep 'Cpu(s)' | awk '{print $4}')";
    }
}

if (!function_exists('getDiskInfo')) {
    function getDiskInfo() {
       echo "Disk Size: $(df -h / | awk 'NR==2 {print $2}') / Disk Used: $(df -h / | awk 'NR==2 {print $3}')";
   }
}
